...
export default function Detail() {
    ...
    const [toLevel5, setToLevel5] = useState(false);
    ...
    const GetActionFormat = (cell, row) => {
        ...
        if (row.regstatus === "Reject" || row.regstatus === "Blacklist") {
            a = (<Button color="warning" size="sm" disabled={selectedPeriod.active !== "Active"||row.currentlevel !== 4}
                onClick={e => {
                    e.stopPropagation();
                    toggleUndo(row);
                }}>
        ...
        return (
            <div>
                <Button color="primary" className="mr-2" size="sm"
                    onClick={e => {
                        e.stopPropagation();
                        history.push("/tahap4/req", { req: row, active: selectedPeriod.active === "Active" })
                    }}>
                    <FontAwesomeIcon icon={['fa', 'info-circle']} />
                </Button>
                {a}
    ...
    function statusFormatter(cell, row) {
        ...
        if (cell === "Approve") {
            return (<span className="text-success">Diterima ke Tahap V</span>);
        }
        if (cell === "Blacklist") {
    ...
    async function fetchData(param) {
        ...
        axios.post('/b/o/trans/admreq', JSON.stringify(param)).then(({data}) => {
          if (data.st) {
    ...
    const handleUndo = (e) => {
      e.preventDefault();
      toast.dismiss();
      axios.post('b/o/leveltwo/undo', {regid: selected.id}).then(({data}) => {
        if(data.st){
    ...
    const level5Handler = () => {
      axios.post('/b/o/levelthree', JSON.stringify({
        regid: selected.registerid
      })).then(({data}) => {
        if(data.st){
          toast.success("Pendaftar berhasil dinaikan ke tahap V", { containerId: 'B', transition: Zoom })
          toggleLevel5();
        }
      }).catch(error => {
        toast.error(Errormsg['500'], { containerId: 'B', transition: Zoom });
    ...
    const submitStatus = async () => {
      axios.post('/b/o/registerstatus', JSON.stringify(regStatusForm)).then(({data}) => {
        if(data.st){
    ...
    return (
        <>
            <Modal zIndex={2000} centered isOpen={toLevel5} toggle={toggleLevel5}>
                <ModalHeader toggle={toggleLevel5}>Apakah anda yakin untuk menaikan {selected.name} ke tahap V ?</ModalHeader>
                <ModalFooter>
                    <Button color="primary" onClick={submitStatus}>Ya</Button>
                    <Button color="secondary" onClick={toggleLevel5}>Tutup</Button>
                </ModalFooter>
            </Modal>
            ...
            <Modal zIndex={2000} centered isOpen={regStatusModal} toggle={toggleRegStatusModal}>
                ...
                <ModalBody>
                    <FormGroup tag="fieldset">
                        ...
                        {   (selected.level === 4)? (<FormGroup check>
                            <Label check>
