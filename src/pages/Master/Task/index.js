...
export default function ListPendaftar() {
  ...
  const deleteHandler = async () => {
    toast.dismiss();    
    axios.post('app/gerai/task/delete', {id_task: selectedUser.id_task, apikey: apikey}).then(({data}) => {
        if (data.status) {
          // if (page == 1) {
  ...
  return (
    <>
      ...
      <Modal zIndex={2000} centered isOpen={toDelete} toggle={toggleDelete}>
        <ModalHeader toggle={toggleDelete}>Apakah anda yakin untuk menghapus task ini?</ModalHeader>
        <ModalBody>          
          <Row>
            <Col xs={4}>Level</Col>
            <Col xs={8}>{toDelete ? ": " + addCommas(selectedUser.level_task) : ": " + selectedUser.level_task}</Col>
          </Row>
          <Row>
            <Col xs={4}>Kuota</Col>
      ...
      <Card>
        <CardBody>
          ...
          <Button color = "primary" onClick = {() => {history.push('task/edit')} }>+ Tambah Task</Button>
          <div className="my-2">            
