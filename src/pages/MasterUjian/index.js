...
export default function AdminReqList() {
  ...
  const waktuUjian = (cell,row) =>{
    console.log(row)
    let date = new Date(row.datestart);
    return(
      <>
  ...
  function fetchData(param) {
    axios.post('/b/o/master/exam/setup', JSON.stringify(param)).then(res => res.data)
      .then(data => {
        if (data.st) {
          console.log(data)
  ...
  const splitDate= (tanggalMulai) =>
  {
    ...
    var arrTanggal= String(tanggalMulai).split('-');
  ...
  return (
    <>
      <Modal zIndex={2000} centered isOpen={modal} toggle={toggle}>
        ...
        <ModalBody>
          ...
          <Row>
            ...
            <Col xs={6}>{": " + splitDate( selectedReq.datestart)}</Col>
          </Row>
          <Row>
            <Col xs={6}>Tanggal Selesai Ujian</Col>
            <Col xs={6}>{": " + splitDate( selectedReq.dateend)}</Col>
          </Row>          
      ...
      <Card>
        <CardBody>
          <CardTitle>Master Ujian</CardTitle>
          {/* {
            user.tu === "Administrator" && */}
            <div className="mb-2">
              <Button color="primary" size="md" className="mr-2" onClick={(e) => {
                history.push('/master/ujian/setting/edit', { admin: user.tu === "Administrator" })
              }}>
              ...
              {/* <Button color="primary" size="md" className="mr-2" onClick={(e) => {
                history.push('/master/ujian/kategori', { admin: user.tu === "Administrator" })
              }}>
                <span style={{ marginRight: 10 }}>
                  Lihat Kategori
                </span>
              ...
              <Button color="primary" size="md" className="mr-2" onClick={(e) => {
                history.push('/master/ujian/soal', { admin: user.tu === "Administrator" })
              }}>
                <span style={{ marginRight: 10 }}>
                  Lihat Soal
                </span>
              ...
              {/* <Button color="primary" size="md" className="mr-2" onClick={(e) => {
                history.push('/master/ujian/jurusan', { admin: user.tu === "Administrator" })
              }}>
                <span style={{ marginRight: 10 }}>
                  Lihat Jurusan Ujian
                </span>
              ...
              {/* <Button color="primary" size="md" className="mr-2" onClick={(e) => {
                history.push('/master/ujian/jawaban', { admin: user.tu === "Administrator" })
              }}>
                <span style={{ marginRight: 10 }}>
                  Lihat Jawaban
                </span>
          ...
          <BootstrapTable
            ...
            noDataIndication="Belum ada Ujian"
