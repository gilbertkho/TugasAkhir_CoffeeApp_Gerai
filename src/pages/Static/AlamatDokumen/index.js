...
export default function UserEditForm(props) {
  ...
  const _getTatib = async () => {
    ...
    axios.post("/b/o/static/addresstatib").then(({ data }) => {
      if (data.sc == 200) {
        if (data.st) {
  ...
  const _onSubmit = (e) => {
    ...
    if (valid) {
      changeSubmitDisableState(true);
      let url = '/b/o/static/addresstatib/edit';
      axios.post(url, { text: tatib }).then(({ data }) => {
        if (data.sc == 200) {
          if (data.st) {
            toast.success("Alamat berhasil diubah", { containerId: 'B', transition: Zoom });
          } else {
            // console.log("error");
            toast.error(data.msg, { containerId: 'B', transition: Zoom });
          }
    ...
    <>
      <Row>
        <Col sm="12" md={{ size: 6, offset: 3 }}>
          <Card body>
            <CardTitle>Alamat Pengiriman Dokumen</CardTitle>
            <Form>
              <FormGroup>
                <Label>Alamat</Label>
