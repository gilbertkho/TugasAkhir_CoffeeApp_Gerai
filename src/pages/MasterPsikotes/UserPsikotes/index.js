...
export default function UserPsikotesList() {
  ...
  const toEditUser = (user) => history.push('/master/userpsikotes/edit', { user: user });
  ...
  function fetchData(param) {
    axios.post('/b/o/master/users/psikotes', JSON.stringify(param)).then(res => res.data)
      .then(data => {
        if (data.st) {
  ...
  const deleteHandler = async () => {
    toast.dismiss();
    try {
      let res = await axios.post('/b/o/master/users/psikotes/delete', JSON.stringify({
        id: selectedUser.id
      }))
      if (res.data.st) {
        if (page == 1) {
  ...
  return (
    <>
      ...
      <Card>
        <CardBody>
          <CardTitle>Daftar User Psikotes</CardTitle>
          <div className="mb-2">
            <Button color="primary" size="md" className="mr-2" onClick={(e) => { history.push('/master/userpsikotes/edit') }}>
