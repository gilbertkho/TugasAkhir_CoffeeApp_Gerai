...
export default function UserPsikotesEditForm(props) {
  ...
  const _onSubmit = () => {
    ...
    if (user.fullname == '' || user.email == '' || (user.pwd == '' && actionType == 'add') || user.mobile == '') {
      ...
      let url = '/b/o/master/users/psikotes/create';
      let successMsg = 'User Psikotes berhasil ditambahkan';
      if (actionType == 'edit') {
        url = '/b/o/master/users/psikotes/update';
        successMsg = 'User Psikotes berhasil diubah';
      }
        ...
        if (data.sc == 200) {
          if (data.st) {
            ...
            if (actionType == 'add') {
              ...
              setTimeout(
                history.push('/master/userpsikotes/list')
                , 5000);
            }
          } else {
            console.log("error");
      ...
      <Row>
        <Col sm="12" md={{ size: 6, offset: 3 }}>
          <Card body>
            ...
            <CardTitle>{actionType == "add" ? "Tambah User Psikotes" : "Edit User Psikotes"}</CardTitle>
            <Form>
              <FormGroup>
